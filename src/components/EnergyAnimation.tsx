...
const EnergyAnimation = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const growFlowLines = (): void => {
      flowLines.forEach((line) => {
        ...
        line.currentAngle +=
          Math.random() * line.angleRange * 2 - line.angleRange;
    ...
    const draw = (): void => {
      ...
      particles.forEach((particle) => {
        ...
        ctx.arc(
          particle.x,
          particle.y,
          particle.size * (1 + particle.pulse) * (isNearLine ? 2 : 1),
          0,
          Math.PI * 2
        );
        const glowColor = particle.color.replace(
          ")",
          `, ${0.3 - 0.2 * particle.pulse})`
        );
        ...
        ctx.arc(
          particle.x,
          particle.y,
          particle.size * (isNearLine ? 1.5 : 1),
          0,
          Math.PI * 2
        );
  ...
  return (
    <>
      ...
      <div
        className="fixed top-0 left-0 w-full h-full bg-gradient-to-b from-black/30 to-black/70"
        style={{ zIndex: -5 }}
      />
    </>
  );
